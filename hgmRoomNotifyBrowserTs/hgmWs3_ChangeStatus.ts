...
* 数据类型见 hgmWs3_types.ts, 线协议编解码见 hgmWs3_protocol.ts.
 */
import {hgmAsyncSleep} from "./hgmTsWeb_async.ts";
import {hgmPath_Join, hgmPath_RemoveLastSection} from "./hgmTsWeb_path.ts";
import {
    hgmWs3_ObsEvent_t,
    hgmWs3_ClientDeny_t,
    hgmWs3_RoomOnChange_t,
    hgmWs3_ObsDefaultFn,
    hgmWs3_ObsEventType_clientConnDialing,
    hgmWs3_ObsEventType_clientConnConnected,
    hgmWs3_ObsEventType_clientConnClose,
    hgmWs3_ObsEventType_clientServerCloseConn,
    hgmWs3_ObsEventType_clientNeedManual,
    hgmWs3_UiStatusToUser_synced,
    hgmWs3_UiStatusToUser_syncing,
    hgmWs3_UiStatusToUser_offline,
    hgmWs3_UiStatusToUser_needManual,
} from "./hgmWs3_types.ts";
import {
    hgmWs3_Cmd_ping,
    hgmWs3_Cmd_setTimeCfg,
    hgmWs3_Cmd_roomEnter,
    hgmWs3_Cmd_roomLeave,
    hgmWs3_Cmd_roomValue,
    hgmWs3_Cmd_connAllow,
    hgmWs3_Cmd_deny,
    hgmWs3_Cmd_closeConn,
    hgmWs3_DenyScope_conn,
    hgmWs3_marshalPing,
    hgmWs3_marshalIdentity,
    hgmWs3_marshalRoomCmd,
    hgmWs3_readUint16LE,
    hgmWs3_readInt64LEAsMs,
    hgmWs3_readStr16LE,
    hgmWs3_readBytes16LE,
    hgmWs3_readUvarint,
    hgmWs3_decodeUtf8,
} from "./hgmWs3_protocol.ts";

// 默认全局变量实现
export function hgmWs3_ChangeStatusD():WssChangeStatus3_t{
    if (g_hgmWs3_ChangeStatusD!=null){
        return g_hgmWs3_ChangeStatusD
    }
    g_hgmWs3_ChangeStatusD = new WssChangeStatus3_t()
    return g_hgmWs3_ChangeStatusD
}

export class WssChangeStatus3_t{
    setUrl(url:string){
        url = hgmWs3_handleUrl(url)
        this.url = url
        return this
    }
    // 加入房间. 返回离开函数.
    roomEnter(roomId:string,onChangeFn:(ev:hgmWs3_RoomOnChange_t)=>void):(()=>void){
        ...
        if (room==null){
            room = new hgmWs3_room_t()
            this.roomMap.set(roomId,room);
            this._sendRoomEnterMsg(roomId)
        }else{
            // 已有房间,直接回调当前版本.
            const ev:hgmWs3_RoomOnChange_t = {
                RoomId: roomId,
    ...
    GetSinceLastServerConfirm():number{
        ...
        return hgmWs3_nowUnixMilli() - this.lastReadSuccTime
    }
    ...
    // 观测事件回调. null 表示使用 hgmWs3_ObsDefaultFn. 设置为空函数表示关闭观测.
    ObsFn: ((ev: hgmWs3_ObsEvent_t) => void) | null = null
    ...
    OnDenyFn: ((ev: hgmWs3_ClientDeny_t) => void) | null = null
    ...
    private sendKeepAliveTimer:hgmTimer_t
    private noNeedIdleCloseTimer:hgmTimer_t
    private readToReconnectTimer:hgmTimer_t
    private wsDialTimer:hgmTimer_t
    private timeoutCfg = new hgmWs3_TimeoutCfg_t()
    private roomMap= new Map<string,hgmWs3_room_t>()
    ...
    private _callOnChangeFnSafe(fn:(ev:hgmWs3_RoomOnChange_t)=>void, ev:hgmWs3_RoomOnChange_t){
    ...
    GetUiStatusToUser():string{
        ...
        if (sinceLastRead >= this.timeoutCfg.ClientLastReadToReconnectDur || this.onChangeRunningCount>0 || !isWsConnected){
            return hgmWs3_UiStatusToUser_syncing
        }
        return hgmWs3_UiStatusToUser_synced
    }
    _init_afterEnter(){
    ...
    _postRead(){
        this.lastReadSuccTime = hgmWs3_nowUnixMilli()
    ...
    async _tryConnOnceSync(){
        ...
        // 控制开始连接的最小时间间隔。
        const dur = this.timeoutCfg.ClientReconnectMinDur - (hgmWs3_nowUnixMilli()-this.lastStartConnectTime);
        if (dur>0){
            await hgmAsyncSleep(dur)
        }
        if (this.HasNeed()==false){
            return;
        }
        this.lastStartConnectTime = hgmWs3_nowUnixMilli();
        this.lastKeepAliveSendTime = hgmWs3_nowUnixMilli();
        this.wsDialNum++;
        this._obsCloseReason = ""
        this._emitObs({ Type: hgmWs3_ObsEventType_clientConnDialing, CloseReason: "", CloseDetail: "" })
        ...
        thisSocket.addEventListener("open",()=>{
            ...
            this.lastKeepAliveSendTime = hgmWs3_nowUnixMilli()
            this.lastReadSuccTime = hgmWs3_nowUnixMilli()
            this.lastWriteSuccTime = hgmWs3_nowUnixMilli()
            this._resetClientIdleToSendKeepAlive()
            this.readToReconnectTimer.reset(this.timeoutCfg.ClientLastReadToReconnectDur);
            this._emitObs({ Type: hgmWs3_ObsEventType_clientConnConnected, CloseReason: "", CloseDetail: "" })
            // 本次连接还没认证通过. 先发 identity(永远发), 等 Cmd_connAllow 后再发 roomEnter.
            this.connApproved = false
            this._sendBinaryMsg(hgmWs3_marshalIdentity(this.identity))
        })
        // 等待 socket 关闭.
        await new Promise<void>((resolve)=>{
            thisSocket.addEventListener("close",()=>{
                ...
                this.noNeedIdleCloseTimer.stop()
                this._emitObs({ Type: hgmWs3_ObsEventType_clientConnClose, CloseReason: this._obsCloseReason, CloseDetail: "" })
                resolve();
            });
        })
    }
    ...
    _processMessages(data:Uint8Array){
        ...
        while(pos<data.length){
            ...
            const frameLen = hgmWs3_readUint16LE(data,pos)
            pos+=2
    ...
    _processOneMsg(data:Uint8Array){
        ...
        const cmd = data[0]
        switch(cmd){
        case hgmWs3_Cmd_ping:
            // 服务端回复的ping,不需要处理.
            break
        case hgmWs3_Cmd_setTimeCfg:{
            if (data.length<2){
                this._close_thisSocket("protocolError")
                return
            }
            ...
            const cfg = new hgmWs3_TimeoutCfg_t()
            for (let i = 0; i < count; i++){
                const fieldId = data[p]; p++
                const val = hgmWs3_readInt64LEAsMs(data, p); p += 8
                switch (fieldId){
        ...
        case hgmWs3_Cmd_roomValue:{
            let p = 1
            const rv = hgmWs3_readStr16LE(data,p)
            ...
            const roomEpoch = roomEpochLen>0 ? hgmWs3_decodeUtf8(data.subarray(p,p+roomEpochLen)) : ""
            p+=roomEpochLen
            // ChangeSeq: uvarint
            const uvr = hgmWs3_readUvarint(data,p)
            ...
            const cVersionId = cVersionIdLen>0 ? hgmWs3_decodeUtf8(data.subarray(p,p+cVersionIdLen)) : ""
            p+=cVersionIdLen
            // LiveData: uint16LE长度+内容
            const rv3 = hgmWs3_readBytes16LE(data,p)
        ...
        case hgmWs3_Cmd_connAllow:{
            // [uint8 AuthEnabled]
            const authEnabled = data.length>1 ? data[1]!==0 : false
            this.connDenyLocal = false
            if (authEnabled && this.OnDenyFn===null){
                this._emitObs({ Type: hgmWs3_ObsEventType_clientNeedManual, CloseReason: "", CloseDetail: "server has auth enabled but OnDenyFn is null" })
            }
        ...
        case hgmWs3_Cmd_deny:{
            ...
            if (rv===null){ this._close_thisSocket("protocolError"); return }
            const roomId = rv.s; p = rv.pos
            const rv2 = hgmWs3_readStr16LE(data,p)
            ...
            if (this.OnDenyFn===null){
                ...
                this._emitObs({ Type: hgmWs3_ObsEventType_clientNeedManual, CloseReason: "", CloseDetail: this.needManualMsg })
                this.SetIsStopListen(true)
                return
            }
            if (scope===hgmWs3_DenyScope_conn){
        ...
        case hgmWs3_Cmd_closeConn:{
            ...
            if (isTemp){
                ...
                this.SetIsStopListen(true)
                this._emitObs({ Type: hgmWs3_ObsEventType_clientServerCloseConn, CloseReason: "", CloseDetail: reason })
            }
            break
        }
        }
    ...
    private _emitObs(ev: hgmWs3_ObsEvent_t){
        const fn = this.ObsFn ?? hgmWs3_ObsDefaultFn
        if (fn != null){
            fn(ev)
        }
    }
    constructor(){
        this.sendKeepAliveTimer = hgmTimer(()=>{
            this.lastKeepAliveSendTime = hgmWs3_nowUnixMilli()
            this._sendBinaryMsg(hgmWs3_marshalPing())
        })
        this.noNeedIdleCloseTimer = hgmTimer(()=>{
            this._close_thisSocket("noNeed")
        })
        this.readToReconnectTimer = hgmTimer(()=>{
            this._close_thisSocket("readTimeout")
        })
        this.wsDialTimer = hgmTimer(()=>{
    ...
    _sendRoomEnterMsg(roomId:string){
        ...
        this._sendBinaryMsg(hgmWs3_marshalRoomCmd(hgmWs3_Cmd_roomEnter,roomId))
    }
    _sendRoomLeaveMsg(roomId:string){
        this._sendBinaryMsg(hgmWs3_marshalRoomCmd(hgmWs3_Cmd_roomLeave,roomId))
    }
    _resetClientIdleToSendKeepAlive(){
        let now = hgmWs3_nowUnixMilli()
...
function hgmTimer(fn:()=>void):hgmTimer_t{
...
declare interface hgmTimer_t{
    reset:(dur:number)=>void
    stop:()=>void
}
let g_hgmWs3_ChangeStatusD:WssChangeStatus3_t|undefined = undefined

class hgmWs3_TimeoutCfg_t{
...
class hgmWs3_room_t{
    ...
    ChangeSeq:number = 0
    CVersionId:string = ""
    onChangeSet= new Set<(ev:hgmWs3_RoomOnChange_t)=>void>
}
/*
* 支持三种输入:
    * wss://xxx.com/xxx
    ...
    * /xxx/xxx
 */
function hgmWs3_handleUrl(url:string):string{
    if (url.includes("://")){
        return url;
    }
    let outUrl = ""
    ...
    const path = hgmPath_RemoveLastSection(location.pathname)
    return outUrl+hgmPath_Join(path,url)
}
function hgmWs3_nowUnixMilli():number{
