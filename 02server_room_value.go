...
// 超过 frame 上限时拆成 [Cmd_roomValueMore...][Cmd_roomValueEof] 多条, 由写缓冲整组原子写入,
// 保证同连接上这组分片连续到达、中间不插入其它消息, 客户端按顺序累积分片即可重组.
...
// 把一次大 LiveData 按 maxFrame 切成 roomValueMore...roomValueEof 序列.
// 分片大小取 maxFrame 减去 Eof 帧(携带全部元数据)的固定开销, 保证每条 More/Eof 都不超过 maxFrame.
// 最后一片用 Eof 携带元数据(RoomId/RoomEpoch/ChangeSeq/CVersionId), 其余用 More 只带分片.
func buildRoomValueChunks(roomEpoch string, changeSeq uint64, roomId string, cVersionId string, liveData []byte, maxFrame int) []Msg_t {
	// Eof 固定开销: 1(cmd) + (2+roomId) + (1+epoch) + uvarint(changeSeq) + (1+cVersionId) + 2(分片长度前缀).
	eofMeta := 1 + (2 + len(roomId)) + (1 + len(roomEpoch)) + getUvarintOutputSize(changeSeq) + (1 + len(cVersionId)) + 2
	chunkSize := maxFrame - eofMeta
	if chunkSize < 1 {
		chunkSize = 1
	}
